...
class BoxContainer(BaseContainer):
    ...
    def make_header_and_footer(self):
        ...
        if self.header:
            x_offset = justify_offsets[self.header_justify](self.header)
            self.header_widget = Textfield(self.parent,
                                           relx=self.relx + x_offset,
                                           rely=self.rely,
                                           max_width=len(self.header) + 1,
                                           value=self.header,
                                           color=self.header_color)
        else:
            self.header_widget = None
        if self.footer:
            x_offset = justify_offsets[self.footer_justify](self.footer)
            self.footer_widget = Textfield(self.parent,
                                           relx=self.relx + x_offset,
                                           rely=self.rely + self.height - 1,
                                           max_width=len(self.footer) + 1,
                                           value=self.footer,
    ...
    def resize_header_footer(self):
        max_text_width = self.width - 3
        justify_offsets = {'left': lambda x: 1,
                           'center': lambda x: (self.width - len(x)) // 2,
                           'right': lambda x: (self.width - len(x)) - 1}
        ...
        if self.header:
            x_offset = justify_offsets[self.header_justify](self.header)
            self.header_widget.relx = self.relx + x_offset
            self.header_widget.rely = self.rely
            self.header_widget.max_width = max_text_width

        if self.footer:
            x_offset = justify_offsets[self.footer_justify](self.footer)
            self.footer_widget.relx = self.relx + x_offset
            self.footer_widget.rely = self.rely + self.height - 1
            self.footer_widget.max_width = max_text_width
